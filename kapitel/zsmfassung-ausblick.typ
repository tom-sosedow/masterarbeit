...
// pfadplanung
Bei der Planung des Roboterpfades lagen die zentralen Herausforderungen bei der Bestimmung der Position und Abfolge der Wegpunkte sowie in der Vermeidung von Kollisionen zzwischen Werkzeug,@UE und bereits verlegten Garnstreben. Für ersteres wurden vier verschiedene Arten von Umlenkungen definiert. Diese werden abhängig von der Position des zu umfahrenden @UE ausgewählt und in Relation zur Position des nachfolgenden @UE entsprechend rotiert und gespiegelt. Zur Kollisionsvermeidung wurden drei Ansätze konzipiert, wobei der gezielte Einsatz einer der Umlenkungsarten bereits den Großteil der Kollisionen mit @UE verhindern kann. Kollisionen mit bereits verlegtem Garn werden durch ein gezieltes Anheben des Werkzeugs an berechneten Schnittpunkten zwischen dem geplanten Pfad und einer Approximation des unter Spannung stehenden Garns vermieden. Während der berechnete Pfad zumeist allen Anforderungen entspricht, kann es zu Problemen kommen, falls die zugrunde liegende Route von den Erwartungen abweicht. Entsprechend sind weiterführende Sicherheitsmechanismen notwendig, um einen reibungslosen Ablauf garantieren zu können. 

Die Kombination der drei implementierten Teillösungen erweist sich als äußerst effizient und robust für die vollautomatisierte Herstellung gleichmäßiger Textilbewehrungen. Dabei wird minimale Materialverschwendung erreicht, es sind keine manuellen Eingriffe zwischen Start- und Endknoten erforderlich und durch die Kollisionserkennung der Pfadplanung ist das Werkzeug des Roboters in der Regel geschützt. 

#todo[Wenn Garntest fertig, hier nochmal ergebnisse einbeziehen, dass das auch praktisch alle sgeklappt hat]
