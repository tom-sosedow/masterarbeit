...
Die Methode liefert zuverlässig zulässige Ergebnisse, da sie ohne Zufallskomponenten auskommt. Allerdings ist nicht in jedem Fall gewährleistet, dass die resultierenden Anordnungen optimal für die Lösung des Problems sind. Insbesondere kann die feste Platzierung des ersten Umlenkelements an der linken unteren Ecke der Tür dazu führen, dass weniger @UE abgelegt werden, als es von einem anderen Startpunkt aus möglich wäre, oder Sonderstellen an für die Routenplanung ungünstigen Positionen auftreten. 
...
// 4. Verbesserungspotential
Der Prozess könnte verbessert werden, indem direkt mit Koordinaten in Millimetern gearbeitet werden würde. Neben der Platzierung anderer Bauteile könnte es die Integration von diagonal verlaufenden Wand- oder Türseiten ermöglichen. Außerdem könnten so gegebenenfalls @UE:pl:long verschiedener Größe genutzt werden, um an Stellen, an denen keine hohen Zugkräfte zu erwarten sind, weniger Carbongarn zu verlegen und somit Einsparungen von Materialkosten zu ermöglichen. 

Ferner könnte der Prozess so angepasst werden, dass die Orte der Sonderumlenkungen zu Beginn statisch festgelegt und deren @UE zuerst platziert werden. Die restlichen @UE können anschließend auf Basis dessen regelkonform platziert werden. Dadurch könnte die Routenplanung maßgeblich verändert und deutlich erleichtert werden, da sich die Abfolge der Teilrouten somit in mehr Wandkonfigurationen gleicht und gegebenenfalls kostengünstigere Routen effizienter berechnet werden können.

Außerdem wurden in dieser Arbeit keine Ansätze betrachtet, bei denen die Platzierung der Umlenkelemente kooperativ und parallel zur Routen- bzw. Pfadplanung geschieht. Weiterführende Arbeiten könnten Möglichkeiten erforschen, welche zuerst eine optimierte Gitterstruktur in den vorgegebenen Maßen generieren und anschließend die nötigen Stellen identifizieren, an denen @UE:pl:long platziert werden müssen, um jene Gitterstruktur zu erzeugen.

// 5. Fazit
Die behandelten Verfahren zur Routen- und Pfadplanung bauen auf den hier bestimmten Positionen der @UE:pl auf. Es konnte gezeigt werden, dass sich auf dieser Grundlage gleichmäßige und den Anforderungen entsprechende Bewehrungsmatten in einem angemessenen zeitlichen Rahmen erzeugen lassen. Somit stellt das gezeigte iterative Platzierungsverfahren insgesamt eine geeignete Antwort auf die Forschungsfrage (I) dar.