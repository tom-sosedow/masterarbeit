...
// 1. Ziel und Kontext
Zur Beantwortung der Forschungsfrage (III) "_#forschungsfragen.at(2)_" wurde ein Algorithmus zur Planung eines Pfades entwickelt und implementiert. Dieser generiert Wegpunkte auf Basis der zuvor erstellten Route zwischen den Umlenkelementen. Die Beantwortung der Forschungsfrage ist essentiell für das Hauptproblem, da ohne eine gute Pfadplanung kein Carbongitter, welches die strukturellen Anforderungen erfüllt, vollautomatisiert erstellt werden kann.
...
Insbesondere bei der Erzeugung von Kontaktpunkten zwischen sich kreuzenden Streben spielt dieser Vorteil bezüglich der Garnspannung eine wichtige Rolle. In den durchgeführten Tests konnte der nötige vertikale Anstieg in der Nähe der @UE im Mittel gegenüber dem aktuell im @CBT eingesetzten Verfahren reduziert werden, da das Werkzeug erst angehoben wird, sobald eine kreuzende Strebe erreicht wird. Die Spannung aller Streben konnte dabei vollständig aufrecht erhalten werden.
...
#question[Forschungsergebnisse aus praktischem Test: Wie zu bewerten? Irgendwie blöd die Schuld am Versagen abzuschieben

-> Probleme aufgezeigt, die später Ingeniuerstechnisch aufgegriffen werden müssen. Waren vorher nicht ersichtlich
]

// Kollisionsvermeidung Garnstreben 
Die Bestimmung der Kreuzungspunkte zwischen dem aktuellen Pfadabschnitt und bereits verlegten Streben basiert jedoch nicht auf einem physikalisch exakten Modell. Entsprechend kann die Genauigkeit dieser Näherung in bestimmten Situationen begrenzt sein. Insbesondere bei vollständigen Umlenkungen an den Ecken der Wand kommt es deshalb zu Problemen, wie bereits in @sec:path-results beschrieben wurde. Aufgrund der Unterschiede zwischen Werkzeugmittelpunkt, welcher den Pfad genau abfährt, und dem gezeigten Werkzeug zur Garnablage wurden bei dem praktischen Test von Wand 5 einige Streben stark gestreift. 
Die beobachteten Ergebnisse zeigen, dass die Kollisionsvermeidung zwischen Werkzeug und Garnstreben noch nicht ausreichend ausgereift ist, um einen zuverlässigen Einsatz in der industriellen Produktion zu gewährleisten. Aufgrund der physikalischen Einschränkungen des Werkzeugs sowie des daraus erforderlichen vertikalen Versatzes zur Kollisionsvermeidung sind die Verbesserungsmöglichkeiten innerhalb des vorgestellten Ansatzes begrenzt. Selbst bei einer Anpassung der Position und Höhe der Wegpunkte für alle Umlenkungsarten müsste entweder auf die Verbindung zwischen sich kreuzender Streben oder auf die Kollisionsfreiheit verzichtet werden, was in beiden Fällen negative Auswirkungen auf die strukturelle Integrität hätte. In Abstimmung mit dem Team des @CBT erscheint daher künftig eine Anpassung sowohl der Wegpunktdefinition als auch des Werkzeugs zur Garnablage als erforderlich.

Die Vermeidung von Kollisionen mit den @UE:pl funktioniert hingegen zuverlässig, sodass für realistische Wandgrößen keine Kollisionen zu erwarten sind. Die Kombination mit dem Ansatz von #citep(<morris-hillBuildingStringArt2023>) erreicht mehr Sicherheit durch klare Terminationsbedingungen und Fehlererkennung. Dadurch ist sichergestellt, dass ausgegebene Pfade nicht zu einem Schaden am Werkzeug oder Roboterarm führen können.
