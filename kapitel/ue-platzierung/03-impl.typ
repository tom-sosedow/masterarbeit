...
In den ersten 15 Zeilen werden dabei Hilfsvariablen angelegt, um die Seite des in @fig:ue-place-step-2 rot markierten @UE zu bestimmen oder ob der Türausschnitt eine gerade Anzahl an @UE breit ist. In Zeile 17 wird der Spaltenindex berechnet, an dem das am weitesten Links stehende @UE an der Oberseite der Wand liegen muss. Basierend darauf muss die y-Komponente der übrigen @UE:pl an den horizontalen Seiten angepasst werden. Diese wird innerhalb der Schleife in Zeilen 20 bis 26 berechnet. Danach steht die Koordinate des zu platzierenden @UE fest. Bevor es zur Menge der Knoten hinzugefügt werden kann, muss allerdings sichergestellt sein, dass in der jeweiligen Spalte kein @UE in der Zeile darunter durch #text(font: "JetBrains Mono", weight: "thin", size: 9pt)[PlaceVertGE] platziert wurde. In @fig:ue-place-step-2 ist dieses Element rot markiert und verhindert somit, dass bei $(tx2,ty1)$ ein @UE platziert wird, da sonst eine Art Sonderstelle entstehen würde.
