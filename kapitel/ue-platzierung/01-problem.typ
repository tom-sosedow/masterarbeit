...
Beim @CBT wurde der zweite Ansatz gewählt, da das in Harz getränkte Garn nach der Temperierung im Ofen sonst an der Ablageoberfläche haften würde. Aus diesem Grund werden zylinderförmige Körper aus Polytetrafluorethylen (PTFE, umgangssprachlich auch Teflon) als @UE:pl:long eingesetzt, von denen die Garnstruktur später leichter zu lösen ist. Diese verfügen über eine magnetische Basis, wodurch sie von einem Roboterarm auf einer ferromagnetischen Platte frei in zwei Dimensionen positioniert werden können. Ein typisches @UE ist in @fig:umlenkelement abgebildet. Sie haben üblicherweise einen Durchmesser von fünf Zentimetern und eine Höhe von zehn Zentimetern, zzgl. zwölf Millimeter für die Basis.
