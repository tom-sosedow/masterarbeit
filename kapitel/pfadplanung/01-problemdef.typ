...
Analog dazu kann eine fehlerhafte Festlegung der zugehörigen Wegpunkte sowie ihrer Reihenfolge dazu führen, dass einzelne @UE:pl ausgelassen werden, Lücken in der Gitterstruktur entstehen oder Kollisionen mit benachbarten @UE auftreten.
...
Die Wegpunkte seien beschrieben durch ein Koordinatentupel im Modell der Routenplanung, wobei eine zusätzliche Komponente den Versatz in z-Richtung angibt. Die Grundebene befindet sich bei $z=0$ und ein positiver Versatz verläuft vom Ablagetisch entfernend. Außerdem müssen nun auch reellwertige Komponenten zulässig sein, um eine feingranularere Platzierung zu ermöglichen. Somit ergeben sich die Wegpunkte als Elemente der Menge $W = RR^3$ wobei die Abbildung in die Koordinaten der Echtwelt analog zu @sec:ue-place-problem stattfindet:
$ (x,y,z) in W |-> (x*d + r + p, y*d + r + p, z * d) $